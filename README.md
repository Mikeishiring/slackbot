...
- **Slack-safe formatting** — markdown is translated to Slack's dialect; code spans and fenced blocks pass through untouched so `**pointers**` and `x ** 2` survive intact; and control characters are escaped, so a tool row containing `<!channel>` renders as text instead of paging everyone.
